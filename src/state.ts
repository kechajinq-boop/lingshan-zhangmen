...
export interface BuildingDef {
  ...
  type: 'gather' | 'craft' | 'sell' | 'train' | 'house';
...
export interface GameStateData {
  ...
  totalCrafted: number; totalBreakthroughs: number;
...
export class GameState {
  ...
  constructor(scene: Phaser.Scene, raw: any, faction: FactionId, restored?: GameStateData) {
    ...
    if (restored) {
      ...
      this.data = {
        ...
        totalCrafted: 0, totalBreakthroughs: 0,
  ...
  speedOf(b: PlacedBuilding): number {
    ...
    const def = this.buildingDef(b.defId);
    if (def.type === 'craft') mult += this.factionBonus('craftSpeed') + this.elderBonus('craftSpeed');
    return mult;
  }
  ...
  workersBonus(b: PlacedBuilding): number {
    ...
    for (const did of b.assigned) {
      ...
      if (def.type === 'gather') attribute = dis.planting;
      else if (def.type === 'craft') attribute = dis.alchemy;
      else if (def.type === 'sell') attribute = dis.business;
