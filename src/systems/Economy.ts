...
export class Economy {
  ...
  tickBuilding(b: PlacedBuilding, dt: number): void {
    ...
    if (def.type === 'gather' || def.type === 'mine') {
      b.progress += (dt / def.baseTime) * speed;
      if (b.progress >= 1) {
        b.progress = 0;
        if (def.id === 'lingkuang') {
          d.spiritOre++; this.oreCount++; this.gs.events.emit('float', b, '+1灵矿石', 0x75d9d1);
        } else {
          d.herbs++; this.gatherCount++; this.gs.events.emit('float', b, '+1药草', 0x7ddb6a);
        }
      }
    } else if (def.type === 'craft' || def.type === 'forge') {
      if (def.id === 'lianqi') {
        if (d.spiritOre >= 3) {
          b.progress += (dt / def.baseTime) * speed;
          if (b.progress >= 1) {
            b.progress = 0; d.spiritOre -= 3; d.azureEdgeSwords++; d.totalForged++; this.forgeCount++;
            this.gs.events.emit('float', b, '+1青锋剑', 0x78c9ef);
          }
        }
        return;
      }
  ...
  sellBuildings(): PlacedBuilding[] {
    return this.gs.data.buildings.filter(b => {
      const type = this.gs.buildingDef(b.defId).type;
      return type === 'sell' || type === 'artifactSell';
    });
  }
  ...
  shopCanSell(shop: PlacedBuilding): boolean {
    const d = this.gs.data;
    if (shop.defId === 'faqipu') return d.azureEdgeSwords > 0;
    if (shop.sellRecipe) return (d.pills[shop.sellRecipe] || 0) > 0;
    return this.totalPills() > 0;
  }

  totalShopStock(shops: PlacedBuilding[]): number {
    const hasPillShop = shops.some(shop => shop.defId === 'danpu');
    const hasArtifactShop = shops.some(shop => shop.defId === 'faqipu');
    return (hasPillShop ? this.totalPills() : 0) + (hasArtifactShop ? this.gs.data.azureEdgeSwords : 0);
  }
  ...
  notifyShopStockShortage(shops: PlacedBuilding[], sellable: PlacedBuilding[]): void {
    ...
    if (this.totalShopStock(shops) === 0 || this.totalShopStock(shops) < shops.length || sellable.length < shops.length) {
      this.lastShopStockNoticeDay = d.day;
      this.gs.logEvent(
        'visitor-leave',
        'info',
        '商铺客流不足',
        '丹药或青锋剑库存不足，部分商铺暂时无客，补足对应商品后会自动分流。',
        '#ffb347',
      );
    }
  }
  ...
  tickVisitors(dt: number): void {
    ...
    for (let i = d.visitors.length - 1; i >= 0; i--) {
      ...
      if (v.state === 'walking') {
        ...
        if (shop.progress >= 1) {
          shop.progress = 0;
          const artifactShop = shop.defId === 'faqipu';
          const recipe = artifactShop ? null : this.pickPillToSell(shop);
          if ((artifactShop && d.azureEdgeSwords > 0) || recipe) {
            const productName = artifactShop ? '青锋剑' : recipe!.name;
            const price = artifactShop ? this.gs.artifactSellPrice() : this.gs.sellPrice(recipe!.id);
            if (artifactShop) d.azureEdgeSwords--;
            else d.pills[recipe!.id]--;
